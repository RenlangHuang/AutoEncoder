...
class DAE(torch.nn.Module):
    def __init__(self, latent=[256,64,8]):
...
def main():
    ...
    x_train, y_train, x_test, y_test = mnist['arr_0'], mnist['arr_1'], mnist['arr_2'], mnist['arr_3']
    ...
    torch.save(model.state_dict(),'./checkpoint/dae.pth')
    plt.plot(losses)
    plt.grid()
    plt.show()
...
if __name__=='__main__':
    #main()
    model = DAE().to(device)
    model.load_state_dict(torch.load('./checkpoint/dae.pth'))
    ...
    #x_noise = x_noise.reshape(x_noise.shape[0],28*28)
    x_test = x_test.reshape(x_test.shape[0],28*28)
    ...
    for i in range(5):
        ...
        plt.subplot(3,5,i+6)
        plt.imshow(x_noise[i], cmap='gray')
